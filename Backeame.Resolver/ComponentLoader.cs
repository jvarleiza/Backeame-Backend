...
namespace Backeame.Resolver
{
    ...
    {
        public static void LoadContainers(IUnityContainer container, string path, string pattern)
        {
            string parentFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, path);

            foreach (var dir in Directory.GetFileSystemEntries(parentFolder, "*", SearchOption.TopDirectoryOnly))
            {
                try
                {
                    var subFolder = Path.GetFileName(dir);
                    var subDir = Path.Combine(path, subFolder);
                    LoadContainer(container, subDir, pattern);
                }
                catch (Exception ex)
                {
                }
    ...
    internal class RegisterComponent : IRegisterComponent
    {
        ...
        public void RegisterType<TFrom, TTo>(TFrom tFrom, TTo tTo, bool withInterception = false)
            where TTo : TFrom
        {
            if (withInterception)
            {
            ...
            {
                this._container.RegisterType(tFrom.GetType(), tTo.GetType(), null, new HierarchicalLifetimeManager());
            }
        }
